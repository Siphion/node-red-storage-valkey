...
const filesystem_helper_js_1 = require("./filesystem-helper.js");
...
class ValkeyStorage {
    ...
    packageSubscriber;
    lastKnownPackages;
    localfilesystem; // Public for git user sync in index.ts
     ...
     */
    async init(settings, runtime) {
        const userConfig = settings.valkey || {};
        // Separate storage-specific options from ioredis connection options
        const { keyPrefix = 'nodered:', publishOnSave = false, subscribeToUpdates = false, updateChannel = 'nodered:flows:updated', enableCompression = false, sessionTTL = 86400, // 24 hours
        supportFileSystemProjects = false, syncPackages = false, packageChannel = 'nodered:packages:updated', packageSyncOnAdmin = false, packageSyncOnWorker = false, enableProjects = true, // Default true for backward compatibility
        ...ioredisConfig } = userConfig;
        // Type-safe ioredis config
        const ioredisOptions = ioredisConfig;
        ...
        const connectionConfig = ioredisOptions;
        ...
        if (settings.userDir && runtime) {
            try {
                // STEP 1: For Admin nodes with Projects, restore project config BEFORE init
                // This ensures Node-RED can activate the correct project during localfilesystem.init()
                if (this.config.enableProjects) {
                    await this.restoreProjectConfigFromRedis(settings);
                }
                // STEP 2: Load and initialize localfilesystem (both Admin and Worker)
                // Dynamically resolve the localfilesystem module path
                const runtimePath = require.resolve('@node-red/runtime');
                const runtimeLibDir = path.dirname(runtimePath);
                ...
                if (this.config.enableProjects) {
                    console.log('[ValkeyStorage] LocalFileSystem initialized for Admin with Projects support');
                }
                else {
                    console.log('[ValkeyStorage] LocalFileSystem initialized for Worker (no Projects)');
                }
                // STEP 3: Restore flows and credentials from Redis
                // Admin: Restore to project directory and update localfilesystem memory
                // Worker: Restore to normal directory and update localfilesystem memory
                await this.restoreFlowFilesFromRedis(settings);
                // STEP 4: For Worker nodes, also populate cache for getFlows/getCredentials
                if (!this.config.enableProjects) {
        ...
        if (this.config.supportFileSystemProjects) {
            ...
            this.fsHelper = new filesystem_helper_js_1.FileSystemHelper(settings.userDir);
        ...
        if (this.config.syncPackages) {
            ...
            this.packageHelper = new package_helper_js_1.PackageHelper(settings.userDir);
            ...
            if (this.config.packageSyncOnWorker) {
                ...
                this.packageSubscriber.on('message', async (channel, message) => {
                    if (channel === this.config.packageChannel) {
                        try {
                            ...
                            const packageList = JSON.parse(message);
            ...
            if (this.config.packageSyncOnAdmin) {
                ...
                if (configData) {
                    try {
                        const configJson = await this.deserialize(configData);
     ...
     */
    async saveFlows(flows, skipPublish = false) {
        // Debug logging
     ...
     */
    async saveCredentials(credentials, skipPublish = false) {
        // Save to Projects (if available) for Git versioning
        if (this.localfilesystem) {
     ...
     */
    async restoreProjectConfigFromRedis(settings) {
        if (!settings.userDir) {
     ...
     */
    async restoreFlowFilesFromRedis(settings) {
        if (!this.localfilesystem) {
            console.warn('[ValkeyStorage] Cannot restore: localfilesystem not initialized');
            return;
        }
        try {
            ...
            const flows = await this.deserialize(flowsData);
            const creds = credsData ? await this.deserialize(credsData) : {};
            console.log('[ValkeyStorage] Restoring flows and credentials from Redis');
            // Use localfilesystem methods to write files AND update memory
            await this.localfilesystem.saveFlows(flows);
            console.log('[ValkeyStorage] Flows restored via localfilesystem');
            await this.localfilesystem.saveCredentials(creds);
            console.log('[ValkeyStorage] Credentials restored via localfilesystem');
        }
        catch (error) {
    ...
    async loadFlowsIntoCache() {
        try {
            ...
            if (credsData) {
                const creds = await this.deserialize(credsData);
    ...
    sanitizeFlows(flowConfig) {
        ...
        if (validFlows.length !== flows.length) {
            console.warn(`[ValkeyStorage] Sanitized flows: ${flows.length} → ${validFlows.length} (removed ${flows.length - validFlows.length} invalid entries)`);
        }
     ...
     */
    async ensurePackageJson(userDir) {
     ...
     */
    async ensureConfigFiles(userDir) {
     ...
     */
    async handlePackageSync(settings) {
        try {
            ...
            throw new Error(`Package synchronization failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
